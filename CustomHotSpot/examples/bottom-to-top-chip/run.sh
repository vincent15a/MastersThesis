...
../../hotspot -c example.config -p PowerTrace.ptrace -grid_layer_file layers.lcf -model_type grid -detailed_3D on -grid_steady_file outputs/example.grid.steady -steady_file outputs/example.steady

cp outputs/example.steady example.init

../../hotspot -c example.config -p PowerTrace.ptrace -grid_layer_file layers.lcf -init_file example.init -model_type grid -detailed_3D on -o outputs/example.transient -grid_transient_file outputs/example.grid.ttrace
