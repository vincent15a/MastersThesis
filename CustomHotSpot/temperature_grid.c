...
void populate_R_model_grid(grid_model_t *model, flp_t *flp)
{
  ...
  #if VERBOSE > 1
	fprintf(stdout, "Resistances: \n");
	for(unsigned int i = 0; i < model->n_layers - 1; i++) {
		fprintf(stdout, "Layer %d: \n", i);
    for (unsigned int j = 0; j < model->rows; j++) {
      for (unsigned int k = 0; k < model->cols; k++) {
        fprintf(stdout, "Resistance [%d][%d]: %f \n", j, k, find_res(model, i, j, k, model->n_layers - 1, j, k));
      }
    }
    fprintf(stdout, "%f \n", model->layers[i].rz);
	}
