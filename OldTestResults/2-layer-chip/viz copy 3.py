...
def create_plot(pairs, xlabel, ylabel, title: str):
    ...
    colors = ['b', 'g', 'r', 'm', 'y']
    labels = ["MILP", "Greedy", "Splitting", "MILP Approximation"]
    c_index = 0
    for x, y in pairs:
...
create_plot([(dfMILP['Nrtasks'], dfMILP['Time']), (dfGreedy['Nrtasks'], dfGreedy['Time']), (dfSplitting['Nrtasks'], dfSplitting['Time']), (dfMILPApprox['Nrtasks'], dfMILPApprox['Time'])], 'Number of tasks', 'Time (s)', 'Running Time')
create_plot([(dfMILP['Nrtasks'], dfMILP['Makespan']), (dfGreedy['Nrtasks'], dfGreedy['Makespan']), (dfSplitting['Nrtasks'], dfSplitting['Makespan']), (dfMILPApprox['Nrtasks'], dfMILPApprox['Makespan'])], 'Number of tasks', 'Makespan', 'Makespan')
create_plot([(dfMILP['Nrtasks'], dfMILP['MaxTemp']), (dfGreedy['Nrtasks'], dfGreedy['MaxTemp']), (dfSplitting['Nrtasks'], dfSplitting['MaxTemp']), (dfMILPApprox['Nrtasks'], dfMILPApprox['MaxTemp'])], 'Number of tasks', 'Maximum Temperature (K)', 'Maximum Temperature')
...
def create_plot6(pairs, xlabel, ylabel, title: str):
    ...
    plt.figure(figsize=(10, 6))
    colors = ['b', 'g', 'r', 'm', 'y', 'c']
    labels = ["TATS-ND-FP", "RA-BC", "RA-BC No Horizontal Dissipation", "TATS-ND-FP Approximation", "RA-BC Approximation", "RA-BC No Horizontal Dissipation Approximation"]
    c_index = 0
    for x, y in pairs:
        plt.plot(x, y, '--', color=colors[c_index], label = labels[c_index])
        c_index += 1
...
#     plt.figure(figsize=(10, 6))
#     colors = ['b', 'g', 'r', 'm', 'y']
#     labels = ["Matrix Model", "RA-BC", "RA-BC No Horizontal Dissipation"]
#     c_index = 0
#     for x, y in pairs:
#         plt.plot(x, y, '--', color=colors[c_index], label = labels[c_index])
#         c_index += 1
...
def create_plot8(pairs, xlabel, ylabel, title: str):
    ...
    plt.figure(figsize=(10, 6))
    colors = ['b', 'g', 'r', 'm', 'y']
    labels = ["TATS-ND-FP", "RA-BC", "RA-BC No Horizontal Dissipation"]
    c_index = 0
    for x, y in pairs:
        plt.plot(x, y, '--', color=colors[c_index], label = labels[c_index])
        c_index += 1
